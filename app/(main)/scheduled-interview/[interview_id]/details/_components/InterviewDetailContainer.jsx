...
//           <h2 className='flex text-md font-bold items-denter gap-2'><Clock className='h-4 w-4'/> {interviewDetail?.duration}</h2>
//         </div>
//         <div>
//           <h2 className='text-sm text-gray-500'>Created on</h2>
//           <h2 className='flex text-md font-bold items-denter gap-2'><Calendar className='h-4 w-4'/> {moment(interviewDetail?.created_at).format('MMM DD yyyy')}</h2>
//         </div>
...
function InterviewDetailContainer({ interviewDetail }) {
  return (
    <div className='p-5 bg-white rounded-lg mt-5'>
      <h2 className='font-bold text-xl'>{interviewDetail?.jobPosition || "N/A"}</h2>
      ...
      <div className='mt-4 flex items-center justify-between lg:pr-52'>
        ...
        <div>
          <h2 className='text-sm text-gray-500'>Created on</h2>
          <h2 className='flex text-md font-bold items-center gap-2'>
            <Calendar className='h-4 w-4'/> {interviewDetail?.created_at ? moment(interviewDetail.created_at).format('MMM DD yyyy') : "N/A"}
          </h2>
        </div>
      ...
      <div className='mt-5'>
        <h2 className='font-bold'>Job Description</h2>
        <p className='text-sm leading-6'>{interviewDetail?.jobDescription || "N/A"}</p>
      </div>
      ...
      <div className='mt-5'>
        ...
        <div className='grid grid-cols-2 gap-3 mt-3'>
          {interviewDetail?.questionList?.map((item, index) => (
            <h2 className='text-xs' key={index}>{index+1}. {item?.question || "N/A"}</h2>
          ))}
        </div>
      </div>
    </div>
